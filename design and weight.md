...
Task 5 uses database-backed customer registration and email/password login with a separate staff security boundary. Customer registration, session restoration, email verification, password recovery, profile security, notification preferences, and saved-property ownership use the authoritative backend. Public users cannot select or inherit staff roles. Customer and staff use different opaque Secure/HttpOnly/SameSite cookie sessions, hashed in MySQL, and never expose bearer tokens to JavaScript. Administration keeps its own atomic one-time Head Admin bootstrap. Email verification/reset tokens are hashed and short lived. There is no approved SMS provider or phone OTP flow, no mock fallback, and no automatic import of browser accounts or favorites.
