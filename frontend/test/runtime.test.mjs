...
test("production frontend configuration separates the public and API origins", () => {
  const source = fs.readFileSync(new URL("../src/config/runtime.ts", import.meta.url), "utf8");
  assert.match(source, /canonicalOrigin = "https:\/\/www\.buyhomeforless\.com"/);
  assert.match(source, /canonicalApiOrigin = "https:\/\/api\.buyhomeforless\.com"/);
  assert.match(source, /canonicalApi = `\$\{canonicalApiOrigin\}\/api\/v1`/);
  assert.match(source, /canonicalMedia = `\$\{canonicalApiOrigin\}\/uploads`/);
