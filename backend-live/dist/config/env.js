...
exports.canonicalUploads = exports.canonicalApi = exports.canonicalOrigin = exports.env = void 0;
...
const envSchema = zod_1.z.object({
    ...
    FRONTEND_ORIGIN: zod_1.z.string().url().default(canonicalOrigin),
    ...
    JWT_SECRET: zod_1.z.string().min(16, "JWT_SECRET must be at least 16 characters").optional(),
    JWT_EXPIRES_IN: zod_1.z.string().default("7d"),
    HEAD_ADMIN_EMAIL: zod_1.z.string().email().optional(),
    HEAD_ADMIN_PASSWORD: zod_1.z.string().min(8).optional(),
    HEAD_ADMIN_NAME: zod_1.z.string().min(1).max(80).optional(),
...
function validateProduction(data) {
    const errors = [];
    if (data.PUBLIC_SITE_ORIGIN !== canonicalOrigin || data.FRONTEND_ORIGIN !== canonicalOrigin)
        errors.push("PUBLIC_SITE_ORIGIN and FRONTEND_ORIGIN must be https://buyhomeforless.com");
    if (data.PUBLIC_API_BASE_URL !== canonicalApi)
        errors.push("PUBLIC_API_BASE_URL must be https://buyhomeforless.com/api/v1");
    if (data.PUBLIC_UPLOAD_BASE_URL !== canonicalUploads)
        errors.push("PUBLIC_UPLOAD_BASE_URL must be https://buyhomeforless.com/uploads");
    for (const [name, value] of [["PUBLIC_SITE_ORIGIN", data.PUBLIC_SITE_ORIGIN], ["FRONTEND_ORIGIN", data.FRONTEND_ORIGIN], ["PUBLIC_API_BASE_URL", data.PUBLIC_API_BASE_URL], ["PUBLIC_UPLOAD_BASE_URL", data.PUBLIC_UPLOAD_BASE_URL]]) {
...
exports.env = { ...envData, JWT_SECRET: envData.JWT_SECRET ?? node_crypto_1.default.randomBytes(32).toString("hex"), UPLOAD_DIR_ABSOLUTE: node_path_1.default.resolve(process.cwd(), envData.UPLOAD_DIR) };
